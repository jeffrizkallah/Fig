export interface Faq {
  question: string;
  answer: string;
}

export const FAQS: Faq[] = [
  {
    question: "Do we have to use AI?",
    answer:
      "No. Plenty of what we build has nothing to do with AI: a dashboard, a system replacing three spreadsheets, an automation that removes a manual step. We use AI where it genuinely helps and leave it out where it doesn't. You'll always know which is which.",
  },
  {
    question: "We're a small business. Aren't we too small for this?",
    answer:
      "Almost nobody is. Smaller businesses often see it faster, because one fixed process is a bigger share of the week. If you're genuinely too small to benefit, we'll say so on the first call.",
  },
  {
    question: "We've bought software before and the team never used it.",
    answer:
      "That's the usual outcome, and it's usually because the tool was built for a generic business instead of yours. We build around how your team already works, and we stay through the part where people learn it, which is the part most companies skip.",
  },
  {
    question: "Our data is a mess and we've got nobody technical.",
    answer:
      "That's most businesses we work with. We work with what you have, design around your existing systems, and handle the technical side so your team stays on their actual jobs.",
  },
  {
    question: "How is this different from just buying software off the shelf?",
    answer:
      "Off-the-shelf is generic and you rent it forever. We build around your specific workflow, connect it to what you already use, and it's yours. And we're accountable for whether it actually works.",
  },
  {
    question: "How much of my team's time will this take?",
    answer:
      "You'd be hiring us to take work off their plate, not add to it. We need a few focused sessions early to understand how things really work. After that, we do the lifting.",
  },
  {
    question: "How long until we see something working?",
    answer:
      "Weeks, not quarters. We'd rather put something small in your hands early and improve it than disappear for six months and unveil something.",
  },
];
